<template>
  <section class="intro">
    ...
    - Jeff Goldblum, <em>Thursday, June 5, 1997</em>
  </section>
</template>
  ...
  methods: {
    getQuote() {
      ...
      for (const section in brokenCommas) {
        if (Math.random() > 0.5) {
          newQuote.push(getRandom(getRandom(quotes).split(',')).replace(
            /^.[a-zA-Z]/,
      ...
      for (const stoppedSection in brokenStops) {
        if (Math.random() > 0.5) {
          evenBetterQuote.push(getRandom(getRandom(quotes)
            .split(/\.\s?|,\s/)
            .filter(exists => !!exists)).replace(
            /^.[a-zA-Z]/,
